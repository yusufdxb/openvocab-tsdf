...
hand-annotated spec.
    ...
    python scripts/sam_quality_sweep.py
"""
...
def _encode(cfg_path: Path, sam_short_edge: int, sam_pps: int, out_path: Path) -> tuple[float, int]:
...
def main() -> None:
    ...
    for scene, cfg_str, spec_str in targets:
        for model, sz, pps in variants:
            out = Path(f"outputs/sweep_{scene}_{model}_{sz}_pps{pps}.npz")
            if out.exists():
                print(f"\n=== {scene} {model} @ {sz}/{pps} (cached) ===")
                dt = -1.0
            else:
                print(f"\n=== {scene} {model} @ short_edge={sz}, pps={pps} ===")
                dt, n_frames = _encode(Path(cfg_str), sz, pps, out)
    ...
    print("\n### SAM-dense quality sweep — office0 & room0, 100 frames each")
    ...
    out_doc_path = out_doc / "sam_quality_sweep_office0.json"
